...
cat ./ABC_REF/Input_files/ABC_families.txt | while read i; do 
  ...
  ~/Applications/trimAl/source/trimal -gappyout -in $x$i.faa.aln -out $x$i.faa.aln.trimm 
...
grep -E -A 1 'DroMel|NezVir|BemTab|DapPul|PolVan|TetUrt' ./Final_outputs/Combined_files/Total_ABC.faa | grep -A 1 'ABCH' | sed '/--/d' > ./phylo/ABCH/ABCH_total.faa
...
~/Applications/trimAl/source/trimal -gappyout -in $x.aln -out $x.trimm 
...
~/Applications/raxml/raxmlHPC-PTHREADS-AVX -f a -x 12345 -p 12345 -N 500 -T $THREADS -m PROTGAMMAAUTO -s $x.phy.phy -n ABCH.nwk -w $H/phylo/ABCH/ -o Ecoli_ARTP_OUT
