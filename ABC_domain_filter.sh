...
grep -E -A 1 "DroMel|TriCas" ./Db_build_temp/Only_ABCs.faa | sed '/--/d' >> $x.faa
~/Applications/Custom_Applications/fasta_seq_len_filter.py $x.faa 200 10000 > $x'_length.faa'
...
~/Applications/trimAl/source/trimal -in $y'.aln' -out $y'.aln.trimm'
...
### run tree
nohup ~/Applications/raxml/raxmlHPC-PTHREADS-AVX -f a -x 12345 -p 12345 -N 100 -T $THREADS -m PROTGAMMAAUTO -s $y'_clean.phy' -n 'Unsorted.nwk' -w $H/Filter/Unsorted_clean &
# cp 
