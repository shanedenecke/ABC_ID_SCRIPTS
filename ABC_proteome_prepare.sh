...
#while read i; do grep $i ./ABC_REF/non_model_proteomes/keys/odb10v0_genes.tab >> ./ABC_REF/non_model_proteomes/Insect_OrthoDB.genes.tab; done < ./ABC_REF/non_model_proteomes/keys/OrthoDB_taxids.tsv
# R script ./SLC_id_scripts/supp/orthodb_table_prepare.R

mkdir ./proteome_clean/clean_fasta
while IFS=$'\t' read -r col1 col2
do
cp ./ABC_REF/non_model_proteomes/orthoDB_fasta/$col1'_0'* ./proteome_clean/clean_fasta/$col2'.fasta'
grep -e "^$col1" ./ABC_REF/non_model_proteomes/keys/odb10v0_genes.tab | cut -f 1,3 > ./proteome_clean/clean_fasta/$col2'_OrthoDB_key.txt'
...
~/Applications/custom/fasta_rename_fast_only_exact.py ./proteome_clean/clean_fasta/$col2'.fasta' ./proteome_clean/clean_fasta/$col2'_OrthoDB_key_DICT.txt' > ./proteome_clean/clean_fasta/$col2'_unigene.faa'
done < $SPEC
